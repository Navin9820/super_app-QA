...
const Booking = () => {
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      try {
        const { bookings, hasEverBooked } = await getUserPorterRequests();
        setBookings(Array.isArray(bookings) ? bookings : []);
        setHasEverBooked(!!hasEverBooked);
      } catch (err) {
  ...
  const isNewUser = !loading && !error && !hasEverBooked;
        ...
        <div className="max-w-xl mx-auto bg-white rounded-xl shadow-md p-6 mt-6">
          ...
          {loading ? (
            ...
            <div className="text-center py-6">
              ...
              <h2 className="text-xl font-bold text-gray-800 mb-2">Welcome to Porter!</h2>
              <p className="text-gray-600 mb-6">
                You haven’t created any bookings yet.<br />
                Let’s get your first delivery started!
              </p>
              <button
                onClick={() => navigate('/porter')}
                className="bg-green-600 text-white py-3 px-6 rounded-lg font-semibold hover:bg-green-700 transition-colors shadow-md"
              >
            ...
            <div className="text-center py-6">
              ...
              <h2 className="text-lg font-semibold text-gray-700 mb-2">No recent bookings</h2>
              <p className="text-gray-500 mb-4">Your past bookings may have been completed or archived.</p>
              <button
                onClick={() => navigate('/porter')}
                    ...
                    {hasOtp(booking) && (
                      <div className="mt-2 p-3 bg-gradient-to-r from-yellow-50 to-orange-50 border border-yellow-200 rounded-lg shadow-sm">
                        <div 
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            toggleOtpVisibility(booking._id);
                          }}
                        >
