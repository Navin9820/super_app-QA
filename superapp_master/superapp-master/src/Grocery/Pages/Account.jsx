...
import { User, Package, Heart, Settings, ShoppingCart, Edit2, Camera, Check, X } from "lucide-react";
...
function Account() {
    ...
    const loadProfileData = useCallback(() => {
        const unifiedProfile = profileService.getProfile();
        const next = {
            name: unifiedProfile.fullName || '',
            email: unifiedProfile.email || '',
            phone: unifiedProfile.phone || '',
            avatar: unifiedProfile.profileImage && typeof unifiedProfile.profileImage === 'string' && unifiedProfile.profileImage.startsWith('data:image/')
                ? unifiedProfile.profileImage
                : '',
        };
