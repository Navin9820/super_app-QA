    ...
    return (
        <div className="relative bg-white p-4 rounded-2xl border border-gray-300 shadow-md w-40 h-[230px] flex flex-col ">
            ...
            <div>
                ...
                {/* Product Image */}
                <div
                    onClick={() => navigate(`/home-food/dishes-list-based-on-category-and-hotel/${product.vendor_id}/${product.category_name}`)}
                    className="relative w-[120px] h-[120px] mx-auto pt-2 flex items-center justify-center overflow-hidden rounded-md">
                    <img
                        src={product.vendor.primary_image}
                        alt={product.vendor.firm_name}
            ...
            <div
                ...
                <div className="flex items-center justify-between w-full mt-1">
                    ...
                    <p className="text-[#684DFF] text-xs font-medium">{product.vendor.delivery_time}</p>

                    <div className="flex items-center">
                        <img src={star} alt="Star" className="w-4 h-4" />
                        <span className="ml-1 text-[#242424] text-xs font-medium">{product.vendor.average_rating}</span>
                    </div>
                </div>

                {/* Product Name */}
                <h3 className="text-[#242424] font-medium text-base truncate w-full">{product.vendor.firm_name}</h3>

                {/* Pricing */}
                <div className="flex items-center space-x-2">
                    <p className="text-[#1FA300] font-semibold text-xs"> {product.vendor.offers}</p>
                </div>
                {/* Pricing */}
            </div>
        </div>
