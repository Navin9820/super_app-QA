...
import { FaEllipsisV, FaEdit, FaEye, FaCheck, FaTimes, FaFileAlt } from 'react-icons/fa';
...
const TaxiDriverTable = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch existing taxi drivers
        const driversResponse = await taxiService.getAllTaxiDrivers();
        if (driversResponse.success) {
          setTaxiDrivers(driversResponse.data);
        }
        ...
        if (pendingResponse.ok) {
          ...
          if (pendingData.data && Array.isArray(pendingData.data)) {
            ...
            taxiRequests = pendingData.data.filter(req => req && req.module_type === 'taxi');
          } else if (pendingData.data && typeof pendingData.data === 'object') {
            // If data is an object, look for taxi_drivers array
            if (pendingData.data.taxi_drivers && Array.isArray(pendingData.data.taxi_drivers)) {
              taxiRequests = pendingData.data.taxi_drivers;
            } else if (pendingData.data.porter_drivers && Array.isArray(pendingData.data.porter_drivers)) {
              // If only porter drivers exist, set empty array
              taxiRequests = [];
            }
          }
          
          console.log('🔍 Filtered taxi requests:', taxiRequests);
          console.log('🔍 Number of taxi requests:', taxiRequests.length);
          
          setPendingRequests(taxiRequests);
        } else {
  ...
  console.log('🔍 Current state:', {
    taxiDrivers: taxiDrivers.length,
  ...
  return (
    <div className="p-4">
      ...
      {/* <div className="mb-4 p-3 bg-gray-100 rounded text-sm">
        ...
        Taxi Drivers: {taxiDrivers.length} | 
      ...
      <div className="mb-6">
        ...
        {pendingRequests.length === 0 ? (
          <div className="p-4 bg-yellow-50 border border-yellow-200 rounded text-center text-yellow-700">
            ...
            <br />
            {/* <small>This could mean: 1) All requests are processed, 2) API connection issue, 3) No taxi requests exist</small> */}
          </div>
        ) : (
                    ...
                    <tr key={request._id || request.id || Math.random()} className="border-b border-yellow-200 hover:bg-yellow-100">
                      ...
                      <td className="px-4 py-2 border border-yellow-200">
                        <div className="flex gap-2">
                          <button
                            onClick={() => handleApprove(request._id || request.id, request.module_type || 'taxi')}
                          ...
                          <button
                            onClick={() => handleReject(request._id || request.id, request.module_type || 'taxi')}
        ...
        <table className="min-w-full bg-white border border-gray-200">
          ...
          <tbody>
            {taxiDrivers.length === 0 ? (
              <tr>
                <td colSpan="7" className="px-4 py-2 text-center text-gray-500">No taxi drivers found</td>
              </tr>
            ) : (
              taxiDrivers.map((driver) => (
                <tr key={driver.id} className="border-b hover:bg-gray-50 transition-colors duration-150">
